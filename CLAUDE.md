...
5. **Interaction**: Raycasting + Zustand bridge between Canvas and DOM UI
...
│       ├── DetailOverlay.tsx       # Info card when star/planet clicked
│       └── Loader.tsx
├── hooks/
...
### Interaction System
- **Canvas**: Click handlers on meshes detect instanceId (for stars) or direct mesh (for planets)
- **Store**: Zustand bridge passes CelestialBodyData to UI
...
- Search/filter UI (find specific stars/planets)
